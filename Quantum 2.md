...
>>> timesteps = 100
>>> A = sp.diags([1/(2*a**2),1j/h- 1/a**2,1/(2*a**2)],[-1, 0 ,1],shape=(nodes,nodes))
...
...
/Users/rubenbiesheuvel/anaconda/lib/python3.5/site-packages/scipy/sparse/compressed.py:739: SparseEfficiencyWarning: Changing the sparsity structure of a csr_matrix is expensive. lil_matrix is more efficient.
  SparseEfficiencyWarning)
```

```python
>>> plt.plot(np.absolute(psi[:,0:60:10]))
>>> plt.plot(V/abs(np.min(V)))
...
...
...
>>> A = sp.diags([1/(4*a**2),1/(4*a**2),1j/h- 1/(2*a**2),1/(4*a**2),1/(4*a**2)],[-nodes,-1, 0 ,1,nodes],shape=(nodes**2,nodes**2)).tolil()
>>> A -= 0.5*sp.diags(V,0)
>>> #A[0,-1] = 1/(4*a**2)
... #A[-1,0] = 1/(4*a**2)
... A = A.tocsc()
...
... #Ainv = Ainv.todense()
...
... B = sp.diags([-1/(4*a**2),-1/(4*a**2), 1j/h + 1/(2*a**2), -1/(4*a**2),-1/(4*a**2)],[-nodes,-1,0,1,nodes],shape=(nodes,nodes)).tolil()
>>> B += 0.5*sp.diags(V,0)
>>> #B[0,-1] = -1/(4*a**2)
... #B[-1,0] = -1/(4*a**2)
...
